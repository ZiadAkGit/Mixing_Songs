...
def crossfade_audio(file1, file2, output_file, duration=5):
    """
    Crossfade two MP3 files together.
    
    :param file1: Path to the first MP3 file.
    :param file2: Path to the second MP3 file.
    ...
    ffmpeg.filter([input1, input2], 'acrossfade', d=duration).output(output_file).run(overwrite_output=True)
...
def process_directory(directory, output_directory, bpm_tolerance=5, crossfade_duration=5):
    ...
    for i in range(len(song_list) - 1):
        ...
        print(f"BPM of {os.path.basename(song1)}: {bpm1}")
        print(f"BPM of {os.path.basename(song2)}: {bpm2}")

        if abs(bpm1 - bpm2) <= bpm_tolerance:
            output_file = os.path.join(output_directory, f"combined_{i + 1}.mp3")
            crossfade_audio(song1, song2, output_file, duration=crossfade_duration)
...
# Example usage:
input_dir = 'songs_directory'  # Replace with your directory containing MP3 files
output_dir = 'combined_songs'
process_directory(input_dir, output_dir, bpm_tolerance=5, crossfade_duration=5)
